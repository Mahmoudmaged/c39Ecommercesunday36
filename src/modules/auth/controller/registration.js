...
export const generateRefreshEmailToken = asyncHandler(async (req, res, next) => {
    const { token } = req.params;
    const { email } = verifyToken({ token, signature: process.env.EMAIL_TOKEN })
    if (!email) {
        return next(new Error("In-valid Token payload", { cause: 400 }))
    ...
    return res.status(200).send('<h1>New Confirmation Email have been dent to your inbox please check it ASAP.</h1>')
...
export const sendForgetCode = asyncHandler(async (req, res, next) => {
    ...
    const forgetCode = Math.floor(Math.random() * (9999 - 1000 + 1) + 1000)//4564
    const code = customAlphabet('123456789', 4)
    // console.log(code());
    const user = await userModel.findOneAndUpdate({ email: email.toLowerCase() }, { forgetCode: code() })
    if (!user) {
        return next(new Error("Not Register account or In-valid email", { cause: 404 }))
    }
    const html = `<!DOCTYPE html>
    <html>
    <head>
    ...
    <tr>
    <td>
    <p  style="margin:10px 0px 30px 0px;border-radius:4px;padding:10px 20px;border: 0;color:#fff;background-color:#630E2B; ">${forgetCode}</a>
    </td>
